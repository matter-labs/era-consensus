...
async fn test_resolve() {
    ...
    let addrs = Host("localhost:1234".into()).resolve(ctx).await.unwrap().unwrap();
    ...
    assert!(Host("localhost".into()).resolve(ctx).await.unwrap().is_err());
    ...
    assert!(Host("".into()).resolve(ctx).await.unwrap().is_err());
}
