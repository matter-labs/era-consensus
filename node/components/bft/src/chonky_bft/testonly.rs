...
impl UTHarness {
    ...
    pub(crate) async fn new_replica_new_view(&mut self, ctx: &ctx::Ctx) -> validator::ReplicaNewView {
        validator::ReplicaNewView {
            justification: validator::ProposalJustification::Timeout(self.new_timeout_qc(ctx).await)
        }
    }
