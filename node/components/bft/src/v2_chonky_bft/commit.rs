...
pub(crate) enum Error {
    ...
    InvalidMessage(#[source] validator::v2::ReplicaCommitVerifyError),
...
impl StateMachine {
    ...
    pub(crate) async fn on_commit(
        &mut self,
        ctx: &ctx::Ctx,
        signed_message: validator::Signed<validator::v2::ReplicaCommit>,
    ) -> Result<(), Error> {
        ...
        let commit_qc = self
            ...
            .entry(message.clone())
            .or_insert_with(|| {
                validator::v2::CommitQC::new(message.clone(), &self.config.validators)
            });
        ...
        let weight = commit_qc.signers.weight(&self.config.validators);
