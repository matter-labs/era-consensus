...
use crate::v2_chonky_bft::{testonly::UnitTestHarness, timeout};
...
fn timeout_qc_aggregation() {
    ...
    for k in &setup.validator_keys {
        ...
        let mut commit_qc = validator::v2::CommitQC::new(commit.clone(), &setup.genesis);
        ...
        let vote = validator::v2::ReplicaTimeout {
            view,
            high_vote: None,
...
async fn timeout_yield_new_view_sanity() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        assert_eq!(util.replica.phase, validator::v2::Phase::Timeout);
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        assert_matches!(
            ...
            Err(timeout::Error::InvalidMessage(
                validator::v2::ReplicaTimeoutVerifyError::BadView(_)
            ))
        );
        ...
        assert_matches!(
            res,
            Err(timeout::Error::InvalidMessage(
                validator::v2::ReplicaTimeoutVerifyError::InvalidHighVote(_)
            ))
        );
        ...
        assert_matches!(
            res,
            Err(timeout::Error::InvalidMessage(
                validator::v2::ReplicaTimeoutVerifyError::InvalidHighQC(_)
            ))
        );
...
async fn timeout_weight_different_messages() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        let replica_timeout = util.new_replica_timeout(ctx).await;
        util.replica.phase = validator::v2::Phase::Prepare; // To allow processing of proposal later.
        ...
        let high_qc = util
            .new_commit_qc(ctx, |msg: &mut validator::v2::ReplicaCommit| {
...
async fn replica_timeout_filter_functions_test() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        let msg = util.owner_key().sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaTimeout(replica_timeout.clone()),
        ));
