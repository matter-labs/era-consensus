...
use crate::v2_chonky_bft::{commit, testonly::UnitTestHarness};
...
async fn commit_yield_new_view_sanity() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        assert_eq!(util.replica.phase, validator::v2::Phase::Commit);
        ...
        assert_eq!(util.replica.phase, validator::v2::Phase::Prepare);
        assert_eq!(new_view.view().number, cur_view.next());
        assert_matches!(new_view.justification, validator::v2::ProposalJustification::Commit(qc) => {
...
async fn replica_commit_old() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        replica_commit.view.number = validator::v2::ViewNumber(util.replica.view_number.0 - 1);
        let res = util
...
async fn replica_commit_unexpected_proposal() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        let replica_commit = validator::v2::ReplicaCommit {
            view: util.view(),
...
async fn replica_commit_different_proposals() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        assert_matches!(replica_commit_result.unwrap().msg.justification, validator::v2::ProposalJustification::Commit(qc) => {
            assert_eq!(qc.message.proposal, replica_commit.proposal);
        });
...
async fn replica_commit_filter_functions_test() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        let replica_commit = util.new_replica_commit(ctx).await;
        let msg = util.owner_key().sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaCommit(replica_commit.clone()),
        ));
        ...
        replica_commit_from_view_2.view.number = validator::v2::ViewNumber(2);
        let msg_from_view_2 = util.owner_key().sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaCommit(replica_commit_from_view_2),
        ));
        ...
        replica_commit_from_view_4.view.number = validator::v2::ViewNumber(4);
        let msg_from_view_4 = util.owner_key().sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaCommit(replica_commit_from_view_4),
        ));
        ...
        replica_commit_from_view_3.view.number = validator::v2::ViewNumber(3);
        let msg_from_view_3 = util.owner_key().sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaCommit(replica_commit_from_view_3),
        ));
        ...
        let msg_from_validator_0 = util.keys[0].sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaCommit(replica_commit.clone()),
        ));
        util.send(msg_from_validator_0.clone());

        // Send a msg from validator 1
        let msg_from_validator_1 = util.keys[1].sign_msg(validator::ConsensusMsg::V2(
            validator::v2::ChonkyMsg::ReplicaCommit(replica_commit),
        ));
