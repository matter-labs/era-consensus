...
async fn replica_prepare_old_view() {
    ...
    let replica_prepare = util.new_current_replica_prepare(|_| {});
    let res = util.dispatch_replica_prepare_one(replica_prepare).await;
    assert_matches!(
        res,
...
async fn replica_prepare_not_leader_in_view() {
    ...
    let replica_prepare = util.new_current_replica_prepare(|_| {});
    let res = util
        .dispatch_replica_prepare_one(replica_prepare.clone())
        .await;
...
async fn replica_prepare_non_validator_signer() {
    ...
    util.dispatch_replica_prepare_one(replica_prepare)
        .await
        .unwrap();
    // PANICS:
...
async fn replica_commit_already_exists() {
    ...
    assert!(util
        .dispatch_replica_prepare_one(replica_prepare_one.clone())
        .await
        .is_err());
