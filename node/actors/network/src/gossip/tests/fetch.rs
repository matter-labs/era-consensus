...
use zksync_concurrency::{ctx,scope,limiter,testonly::abort_on_panic};
...
async fn test_simple() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        let (conn,runner) = gossip::testonly::connect(ctx, &cfg, setup.genesis.hash()).await.unwrap();
        s.spawn_bg(async {
        ...
        stream.send(ctx, &rpc::get_block::Req(setup.blocks[0].number())).await.unwrap();
        ...
        loop {
            let mut stream = conn.open_server::<rpc::push_block_store_state::Rpc>(ctx).await.unwrap();
        ...
        let mut stream = conn.open_client::<rpc::push_block_store_state::Rpc>(ctx).await.unwrap();
        stream.send(ctx, &rpc::push_block_store_state::Req(BlockStoreState {
        ...
        stream.send(ctx, &rpc::get_block::Resp(Some(setup.blocks[1].clone()))).await.unwrap();

        tracing::info!("Wait for the client to store that block");
        store.wait_until_persisted(ctx,setup.blocks[1].number()).await.unwrap();
...
async fn test_concurrent_requests() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        for (i,block) in setup.blocks.iter().enumerate() {
            let mut stream = conns[i%conns.len()].open_server::<rpc::get_block::Rpc>(ctx).await.unwrap();
            let req = stream.recv(ctx).await.unwrap();
        ...
        for (i,stream) in streams.into_iter().enumerate() {
            stream.send(ctx,&rpc::get_block::Resp(Some(setup.blocks[i].clone()))).await.unwrap();
        }
        Ok(())
    })
    .await
...
async fn test_bad_responses() {
    ...
    scope::run!(ctx, |ctx, s| async {
        ...
        for resp in [
            ...
            // Malformed block.
            { let mut b = setup.blocks[0].clone(); b.justification = rng.gen(); Some(b) },
        ] {
            ...
            assert_matches!(conn_task.join(ctx).await.unwrap(),Err(mux::RunError::Closed));
        }
        Ok(())
    })
    .await
