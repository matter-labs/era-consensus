...
//         tracing::info!("BlockStore is empty so requesting a batch should return an empty response.");
...
//         store
//             .batches
//             .queue_batch(ctx, setup.blocks[0].clone(), setup.genesis.clone())
//             .await
//             .unwrap();
//         loop {
//             let mut stream = conn
...
//         let mut stream = conn.open_client::<rpc::get_batch::Rpc>(ctx).await.unwrap();
//         stream
//             .send(ctx, &rpc::get_batch::Req(setup.blocks[0].number()))
//             .await
//             .unwrap();
//         let resp = stream.recv(ctx).await.unwrap();
...
//         stream
//             .send(ctx, &rpc::get_batch::Resp(Some(setup.blocks[1].clone())))
//             .await
//             .unwrap();
...
//         let mut streams = vec![];
//         for (i, batch) in setup.blocks.iter().enumerate() {
...
//             stream
//                 .send(ctx, &rpc::get_batch::Resp(Some(setup.blocks[i].clone())))
//                 .await
//                 .unwrap();
//         }
//         Ok(())
