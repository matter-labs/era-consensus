...
fn mux_entry<R:rpc::Rpc>(ctx:&ctx::Ctx) -> (mux::CapabilityId, Arc<mux::StreamQueue>) {
    (R::CAPABILITY_ID, mux::StreamQueue::new(ctx,R::INFLIGHT,limiter::Rate::INF))
}

/// Establishes an anonymous gossipnet connection to a peer.
pub(super) async fn connect(ctx: &ctx::Ctx, peer: &Config, genesis: validator::GenesisHash) -> ctx::Result<(Conn,ConnRunner)> {
    assert!(peer.gossip.dynamic_inbound_limit>0);
    let addr = peer.public_addr.resolve(ctx).await?.context("peer.public_addr.resolve()")?[0];
    ...
    handshake::outbound(ctx, &cfg, genesis, &mut stream, &peer.gossip.key.public()).await.context("handshake::outbound()")?;
    let conn = Conn {
        accept: [
...
impl Conn {
    ...
    pub(super) async fn open_server<R:rpc::Rpc>(&self,ctx: &ctx::Ctx) -> ctx::OrCanceled<ServerStream<R>> {
        Ok(ServerStream(
            self.connect.get(&R::CAPABILITY_ID).unwrap().open(ctx).await?,
            std::marker::PhantomData,
        ))
    }

    /// Opens a client-side stream.
    pub(super) async fn open_client<R:rpc::Rpc>(&self,ctx: &ctx::Ctx) -> ctx::OrCanceled<ClientStream<R>> {
        Ok(ClientStream(
            self.accept.get(&R::CAPABILITY_ID).unwrap().open(ctx).await?,
            std::marker::PhantomData,
        ))
    }
}
...
impl<R: rpc::Rpc> ClientStream<R> {
    ...
    pub(super) async fn recv(mut self, ctx: &ctx::Ctx) -> anyhow::Result<R::Resp> {
        Ok(frame::mux_recv_proto(ctx, &mut self.0.read, usize::MAX).await?.0)
    }
}
